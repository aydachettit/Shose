...
namespace Shoes.DAL
{
    internal class supplierDAL
    {
        ...
        {
            string query = "INSERT INTO supplier VALUES('"+ autoGenerateSupplierId() + "' , '"  + name + "' , '" + address + "' , '" + phone + "' )";
            return DataProvider.Instance.ExecuteNonQuery(query);
        }

        public int updateSupplier(string id, string name, string address, string phone)
        {
            string query = "UPDATE supplier SET SupplierName = '" + name + "' , SupplierAdress = '" + address + "' , Phone = '" + phone + "' WHERE SupplierID = '" + id + "' ";
            return DataProvider.Instance.ExecuteNonQuery(query);
        }
